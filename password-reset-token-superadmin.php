          ...
          <div class="tab-content" id="nav-tabContent" style="width: 980px;">
            <div class="row">
              ...
              if(isset($_POST['send-link']) && $_POST['email'])
              {
                  include "db.php";  
                  $emailId = $_POST['email'];
                  $result = mysqli_query($con,"SELECT * FROM admintb WHERE email='" . $emailId . "'");
                ...
                { 
                  ...
                  $update = mysqli_query($con,"UPDATE admintb set  reset_link_token='" . $token . "' ,exp_date='" . $expDate . "' WHERE email='" . $emailId . "'");
                  $link = "<a href='http://codadef.com/CODA/reset-password-superadmin.php?key=".$emailId."&token=".$token."'>Click To Reset password</a>";  
                  $mail = new PHPMailer();
                  $mail->CharSet =  "utf-8"; 
                  $mail->IsSMTP();
                  $mail->SMTPAuth = true;                  
                  ...
                  echo "Invalid Email Address. <a href='login'>Go back</a>";
                }
              }
              ?>
                  </div>
